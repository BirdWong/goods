...
<html>
  <head>
	...
	<link rel="stylesheet" href="<c:url value='/menu/adminmenu.css'/>" type="text/css" media="all">
...
function load() {
	...
	bar.colorStyle = 2;//四种样式
	bar.config.imgDir = "<c:url value='/menu/img/'/>";//小工具图片
	bar.config.radioButton=true;//一级排斥
<c:forEach items="${parents}" var="parents">
	<c:forEach items="${parents.children}" var="child">
	 bar.add("${parents.cname}","${child.cname}","/goods/admin/AdminBookServlet?method=findByCategory&cid=${child.cid}","body");
	 </c:forEach>
</c:forEach>
